        ...
        <div class="container-fluid mt--6">
            ...
            <div class="row">
                ...
                <img id="addTaskButton" src="https://img.icons8.com/?size=512&id=24717&format=png" alt="">
                    ...
                    $formattedDueDate = date('d-M-Y', strtotime($value['created_date']));
                    ?>
                                        <li id="task_<?php echo $value['id']; ?>" draggable="true"
                                            ondragstart="dragStart(event)">
                                            <?php echo "<span>" . $value['project_title'] . " <span class='date'>" .$formattedDueDate ."</span></span><br/><b>" . $value['task_title'] . "</b>"; ?>
                                            <img id="task" src="https://cdn.onlinewebfonts.com/svg/img_304350.png"
                                            ...
                                            <a href="#" onclick="editTaskOnClick(<?=$value['id']?>)"><img
                                                    ...
                                                    class="editTaskButton " id="task"></a>
                            ...
                            <div class="task-board">
                                <div class="task-column" id="todo-column">
                                    <h3>To Do</h3>
                                    <ul class="task-list" id="todo-list-set2" ondragover="dragOver(event)"
                                        ondrop="drop_todo(event)">
                                        <!-- Tasks in the "To Do" column will be dynamically added here -->
                                    </ul>
                                </div>

                                <div class="task-column" id="inprogress-column">
                                    <h3>In Progress</h3>
                                    <ul class="task-list" id="inprogress-list-set2" ondragover="dragOver(event)"
                                        ondrop="drop_progress(event)">
                                        <!-- Tasks in the "In Progress" column will be dynamically added here -->
                                    </ul>
                                </div>

                                <div class="task-column" id="completed-column">
                                    <h3>Completed</h3>
                                    <ul class="task-list" id="completed-list-set2" ondragover="dragOver(event)"
                                        ondrop="drop_completed(event)">
                                        <!-- Tasks in the "Completed" column will be dynamically added here -->
                                    </ul>
                                </div>
                                <div class="task-column" id="verified-column">
                                    <h3>Verified</h3>
                                    <ul class="task-list" id="verified-list-set2" ondragover="dragOver(event)"
                                        ondrop="drop_completed(event)">
                                        <!-- Tasks in the "Completed" column will be dynamically added here -->
                                    </ul>
                                </div>
...
function edit(id) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            console.log(data.task.project_id)
            ...
            $('#editTaskForm').attr('action', '<?= base_url("update/") ?>' + data.task.id);
            $('#editTaskForm').show();
        },
        error: function() {
...
$(document).ready(function() 
{
    ...
    const taskBoardSet1 = $('.task-board').eq(0);
    const taskBoardSet2 = $('.task-board').eq(1);

    const noDataMessage = $('#noDataMessage');

    // Initially, hide the second set of task boards
    taskBoardSet2.hide();
    noDataMessage.hide()
    ...
    searchInput.on('input', function()
     {
        const searchQuery = $(this).val(); // Get the user's input
        $.ajax({
            url: '<?= base_url() ?>/search', // Replace with your server's endpoint
            method: 'POST',
            data: {
                query: searchQuery
            },
            ...
            success: function(response)
             {
                ...
                if (response.tasks.length > 0) {
                    // Hide the first set of task boards and show the second set
                    taskBoardSet1.hide();
                    taskBoardSet2.show();

                    // Clear the task lists in the second set
                    $('#todo-list-set2').empty();
                    $('#inprogress-list-set2').empty();
                    $('#completed-list-set2').empty();
                    $('#verified-list-set2').empty();

                    // Update the task lists in the second set with the filtered data
                    response.tasks.forEach(function(task) {
                        const listItem = `
                        <li class="task_${task.id}" draggable="true" ondragstart="dragStart(event)">
                            <span>${task.project_title} - ${task.related_employee} </span><br/><b>${task.task_title}</b>
                            <img class="delete" src="https://cdn.onlinewebfonts.com/svg/img_304350.png" onclick="delete_row(${task.id})" alt="">
                            <a href="" ><img src="http://freevector.co/wp-content/uploads/2012/01/61456-pencil-edit-button.png" data-task-id="${task.id}" class="editTaskButton delete" alt=""></a>
                        </li>
                    `;
                        if (task.status == 0) {
                            $('#todo-list-set2').append(listItem);
                        } else if (task.status == 1) {
                            $('#inprogress-list-set2').append(listItem);
                        } else if (task.status == 2) {
                            $('#completed-list-set2').append(listItem);
                        } else if (task.status == 3) {
                            $('#verified-list-set2').append(listItem);
                        }
...
$(document).ready(function() {
    ...
    function handleFormSubmit(form) {
        ...
        $.ajax({
            type: 'POST',
            url: "<?php echo base_url() ?>/my-task", // URL to the CodeIgniter controller method
            data: formData,
            dataType: 'json',
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    newTaskItem.innerHTML = '<span>' + projectNm + ' - ' + '</span><br/>' +
